...
'<input type="text" id="openai-chat-input" placeholder='+'"' + 'Introduceți întrebarea dvs...' +'"'+ '>' +
...
addChatbotMessage("Buna ziua! Eu sunt chatbot. Cu ce ​​​​vă pot ajuta? (primul mesaj poate dura câteva secunde)");
...
jQuery(document).ready(function($) {
    ...
    async function handleQuestion(questionElement, answerElement) {
        ...
        $.get('https://raw.githubusercontent.com/medanticancerbelievescotland/knowledge/main/knowledge_ro.txt')
            .then(function(data) {
                var knowledge = data;    
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                            ...
                            $(answerElement).append('<div class="loading">Preluare răspuns...</div>');
                        },
                        success: function(response) {
                            $('.loading').remove();
                            if (response.choices && response.choices[0] && response.choices[0].text) {
                                $(answerElement).append('<div class="question-answer"><strong>Întrebare:</strong> ' + question + '<br><strong>Răspuns:</strong> ' + response.choices[0].text + '</div><hr>');
                                ...
                                $(answerElement).append('<div class="error"><strong>Error:</strong> Nu se poate prelua răspunsul.</div><hr>');
                            }
                        },
                        error: function() {
                            $('.loading').remove();
